...
def get_args():
    ...
    parser.add_argument("-scorefile", dest="scorefile", default="score.sc", help="score file") 
...
model_params = data.get_model_haiku_params(model_name=model_name, data_dir = f"{script_path}") # CHANGE THIS (directory where "params/ folder is")
...
def process_output( tag, binderlen, start, feature_dict, prediction_result, scorefilename ):
    ...
    tmpf = f'{outtag}_tmp.pdb.tmp'   
    with open(tmpf, 'w') as f: f.write(unrelaxed_pdb_lines)

    pose = pose_from_file(tmpf)
    pose = insert_chainbreaks( pose, binderlen )
    pose.dump_pdb(tmpf)
    unrelaxed_pdb_path = f'{outtag}.pdb'
    scores_str = "REMARK: sccore:"
    for scorename, value in score_dict.items():
        scores_str += f' {scorename}: {value}'
    with open(tmpf) as f:
        with open(unrelaxed_pdb_path, 'w') as nf:
            nf.write(scores_str+"\n")
            for l in f:
                if l.startswith('ATOM'):
                    resnum = int(l[22:26].strip())
                    plddt = plddt_array[resnum-1]
                    nf.write(l[0:60]+f'{plddt:6.2f}'+l[66:])
                else:
                    nf.write(l)
    os.remove(tmpf)
...
def predict_structure(tag, feature_dict, binderlen, initial_guess, scorefilename, random_seed=0):  
    ...
    process_output( tag, binderlen, start, feature_dict, prediction_result, scorefilename ) 
...
def featurize(tag, pdb_in):
  
    # dump pdb

    tmppdb = pdb_in.split('.pdb')[0]+'_tmp.pdb.tmp'
...
pdbs = []
if args.pdbs.endswith('.pdb') or args.pdbs.endswith('.tmp'):
...
checkpoint_filename = "check.point_af2"
...
for pdb in pdbs:
  ...
  feature_dict, initial_guess, binderlen = featurize(tag, pdb)
  predict_structure(tag, feature_dict, binderlen, initial_guess, scorefilename)
