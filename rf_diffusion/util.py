...
N_BACKBONE_ATOMS = 3
N_HEAVY = 14 
def writepdb_multi(filename, atoms_stack, bfacts, seq_stack, backbone_only=False, chain_ids=None, use_hydrogens=True, idx_pdb=None):
    """
...
def fix_dtypes(specific_contig):
    """
    To make the specific pkl input less annoying, we just check and change the datatypes here.
    """
    out_contig={}
    # first lot need to be lists
    out_contig['ref_idx'] = [(i[0], i[1]) for i in specific_contig['ref_idx']]
    out_contig['hal_idx'] = [(i[0], i[1]) for i in specific_contig['hal_idx']]
    out_contig['idx_rf'] = list(specific_contig['idx_rf'])
    if 'inpaint_seq_array' in specific_contig.keys():
        try:
            out_contig['inpaint_seq_array']=np.array(specific_contig['inpaint_seq_array'])
        except:
            # tensor
            out_contig['inpaint_seq_array']=specific_contig['inpaint_seq_array'].numpy()
    if 'inpaint_str_array' in specific_contig.keys():
        try:
            out_contig['inpaint_str_array']=np.array(specific_contig['inpaint_str_array'])
        except:
            # tensor
            out_contig['inpaint_str_array']=specific_contig['inpaint_str_array'].numpy()
    assert len(out_contig.keys()) == len(specific_contig.keys()), "The only allowed keys in the specific contig input are ref_idx (required), hal_idx (required), idx_rf (required), inpaint_seq_array (optional) and inpaint_str_array (optional)"    
    return out_contig
