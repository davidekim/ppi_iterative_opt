...
class Denoise():
    ...
    def get_next_pose(self,
        ...
        _, ca_deltas = get_next_ca(xt, px0, t, diffusion_mask,
                crd_scale=self.crd_scale, beta_schedule=self.schedule, alphabar_schedule=self.alphabar_schedule, noise_scale=noise_scale_ca)
        ...
        frames_next = get_next_frames(xt, px0, t, diffuser=self.diffuser,
                so3_type=self.so3_type, diffusion_mask=diffusion_mask, noise_scale=noise_scale_frame)
...
class BlockAdjacency():
    ...
    def __init__(self, conf, num_designs):
        ...
        if '-' in str(conf.sampled_insertion):
            self.sampled_insertion = [int(str(conf.sampled_insertion).split("-")[0]), int(str(conf.sampled_insertion).split("-")[1])]
        else:
