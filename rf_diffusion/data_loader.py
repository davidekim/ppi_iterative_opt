...
def get_contacts(complete_chain, xyz_t, cutoff_distance=10):
    ...
    to_mask = random.uniform(0.8,1.0) #randomly mask some proportion of contacting residues 
...
def featurize_single_chain_fixbb(msa, pdb, ss, params, unclamp=False, pick_top=False, fb=False):
    ...
    # process ss
    bb_mask = mask[:,:3].all(dim=1)
    ss, adj = process_ss(ss, xyz.clone().detach(), bb_mask, fb)

    # now expand out to account for nans, which are not included in ss files
    ss_temp = torch.zeros((xyz.shape[0],4)).long()
    ss_temp[bb_mask] = ss.long()
    ss = torch.clone(ss_temp)

    adj_temp = torch.full((xyz.shape[0], xyz.shape[0]), 2).long()
    bb_mask_2d = torch.full((bb_mask.shape[0], bb_mask.shape[0]), True)
    bb_mask_2d[~bb_mask,:] = False
    bb_mask_2d[:,~bb_mask] = False
    adj_temp[bb_mask_2d] = adj.long().reshape(torch.sum(bb_mask)**2)
    adj_temp = adj_temp.reshape(xyz.shape[0], xyz.shape[0])
    adj=torch.clone(adj_temp)
    ...
    return seq.long(), msa_seed_orig.long(), msa_seed.float(), msa_extra.float(), mask_msa, \
           xyz.float(), mask, idx.long(),\
           xyz_t.float(), f1d_t.float(), xyz_prev.float(), \
           chain_idx, unclamp, False, [0,None], ss, adj
...
def loader_pdb_fixbb(item, params, homo = None, unclamp=False, pick_top=False,p_homo_cut=None):
    ...
    #load ss dict
    with open(params["PDB_SS_DIR"]+'/'+item[0][1:3]+'/'+'output.json') as ss_file:
        ss_dict = json.load(ss_file)
    try:
        ss = ss_dict[item[0]]
    except:
        ss=None
    ...
    return featurize_single_chain_fixbb(msa, pdb, ss, params, unclamp=unclamp, pick_top=pick_top, fb=False)
...
def loader_fb_fixbb(item, params, unclamp=False, pick_top=False):
    ...
    #load ss dict
    with open(os.path.join(params["FB_SS_DIR"], item[-1][:2],item[-1][2:],'output.json')) as ss_file:
        ss_dict = json.load(ss_file)
    try:
        ss = ss_dict[item[0]]
    except:
    ...
    return featurize_single_chain_fixbb(msa, pdb, ss, params, unclamp=unclamp, pick_top=pick_top, fb=True)
...
def loader_complex_fixbb(item, L_s, taxID, assem, params, negative=False, pick_top=True):
    ...
    #get ss features
    with open(os.path.join(params['PDB_SS_DIR'], pdbA_id[1:3], 'output.json')) as ss_file:
        ss_dictA = json.load(ss_file)
    with open(os.path.join(params['PDB_SS_DIR'], pdbB_id[1:3], 'output.json')) as ss_file:
        ss_dictB = json.load(ss_file)
    try:
        ssA = ss_dictA[pdbA_id]
        ssB = ss_dictB[pdbB_id]
    except:
        ssA=None
        ssB=None
    ...
    # process ss features
    bb_mask = mask[:,:3].all(dim=1)
    ss, adj = process_complex_ss(ssA, ssB, xyz.clone().detach(), bb_mask)

    # now have to expand them out to account for nans (which are not accounted for in the ss files)
    ss_temp = torch.zeros((xyz.shape[0],4)).long()
    ss_temp[bb_mask] = ss.long()
    ss = torch.clone(ss_temp)

    adj_temp = torch.full((xyz.shape[0], xyz.shape[0]), 2).long() #set to mask
    bb_mask_2d = torch.full((bb_mask.shape[0], bb_mask.shape[0]), True)
    bb_mask_2d[~bb_mask,:] = False
    bb_mask_2d[:,~bb_mask] = False
    adj_temp[bb_mask_2d] = adj.long().reshape(torch.sum(bb_mask)**2)
    ...
    return seq.long(), msa_seed_orig.long(), msa_seed.float(), msa_extra.float(), mask_msa,\
           xyz.float(), mask, idx.long(), \
           xyz_t.float(), f1d_t.float(), xyz_prev.float(), \
           chain_idx, False, False, complete_chain, ss, adj #complete chain is [chainA or B, length of first chain]
...
class DistilledDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        (seq, msa, msa_masked, msa_full, mask_msa, true_crds, atom_mask, idx_pdb, xyz_t, t1d, xyz_prev, same_chain, unclamp, negative, complete_chain, ss, adj) = out
        ...
        assert ss.shape[0] == pop.shape[0], f'chosen dataset {chosen_dataset}'
        ss          = ss[pop]
        adj         = adj[pop2d].reshape(N,N)
        ...
        """
        if 'complex' in self.params['DATASETS']:
            #Added for complex finetuning
            t1d=torch.cat((t1d, torch.zeros(t1d.shape[1])[None,...,None], contacts[None,...,None]), dim=-1)
        ...
        if self.preprocess_param['provide_disulphides']:
            """  
            get disulphide matrix
            Also return 1D map of cyteines that are in disulphides
            """
            disulphides, cysteines=util.get_disulphides(xyz_t[None], seq) 
        else:
            cysteines=None
        masks_1d = generate_masks(msa, task, self.params, chosen_dataset, complete_chain, xyz=xyz_t[0], cysteines=cysteines)
        ...
        if self.preprocess_param['provide_ss']:
            """
            Include SS and/or block adjacency
            Provide each 50% of the time, so 25% of the time, get both, 25% of the time, get none
            """
            if random.uniform(0,1) < 0.5:
                t1d = torch.cat((t1d, ss[None,None].repeat(2,1,1,1)), dim=-1)
            else:
                t1d = torch.cat((t1d, torch.zeros_like(ss)[None,None].repeat(2,1,1,1)), dim=-1)
                t1d[...,-1] = 1
            
            # convert block adj to one_hot: 0 = not adjacent, 1 = adjacent, 2 = masked
            adj = torch.nn.functional.one_hot(adj, num_classes=3).long()
            if random.uniform(0,1) < 0.5:
                t2d = torch.cat((t2d, adj[None,None].repeat(2,1,1,1,1)), dim=-1)
            else:
                t2d = torch.cat((t2d, torch.zeros_like(adj)[None,None].repeat(2,1,1,1,1)), dim=-1)
                t2d[...,-1] = 1.0
        
        if self.preprocess_param['provide_disulphides']:
            """
            Provide disulphide bonds as pairwise feature.
            Provide all disulphides the whole time (as don't want model to build extra ones)
            """
            # convert block adj to one_hot: 0 = not adjacent, 1 = adjacent, 2 = masked
            adj = torch.nn.functional.one_hot(adj, num_classes=3).long()

            if self.preprocess_param['provide_ss']:
                t2d=torch.cat((t2d, disulphides[None,None,...,None].repeat(2,1,1,1,1)), dim=-1)
            else:
                t2d=torch.cat((t2d,torch.zeros_like(adj)[None,None].repeat(2,1,1,1,1),disulphides[None,None,...,None].repeat(2,1,1,1,1)), dim=-1)
