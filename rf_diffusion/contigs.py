...
class ContigMap():
    ...
    '''
    def __init__(self, parsed_pdb, contigs=None, inpaint_seq=None, inpaint_str=None, length=None, ref_idx=None, hal_idx=None, idx_rf=None, inpaint_seq_array=None, inpaint_str_array=None, topo=False, provide_seq=None, inpaint_str_helix=None, inpaint_str_strand=None, inpaint_str_loop=None, **kwargs):
        #sanity checks
        assert not all(x is None for x in [contigs, ref_idx]), "Must either specify a contig string or precise mapping"
        if contigs is None:
            assert all(x is not None for x in [idx_rf, hal_idx, ref_idx]), "If you're specifying specific contig mappings, the reference and output positions must be specified, AND the indexing for RoseTTAFold (idx_rf)"
        ...
        self.inpaint_str_loop=inpaint_str_loop

        self.inpaint_seq_array=inpaint_seq_array
        self.inpaint_str_array=inpaint_str_array
        ...
        if ref_idx is None:
            ...
            self.specific_mappings=False  
        else:
            #specifying precise mappings
            self.specific_mappings=True
            self.ref=ref_idx
            self.hal=hal_idx
            self.rf = idx_rf
            # need to work out which are "receptor" and which are "inpaint" chains
            self.get_inpaint_receptor_chains()
            self.sampled_mask=None
        self.mask_1d = [False if i == ('_','_') else True for i in self.ref]
        #take care of sequence and structure masking
        if self.inpaint_seq_array is None:
            ...
            self.inpaint_seq = self.inpaint_seq_array

        if self.inpaint_str_array is None:
            ...
            self.inpaint_str = self.inpaint_str_array       
    ...
    def get_inpaint_receptor_chains(self):
        """
        If specific contigs have been provided, we need to work out which "chains" are inpainted vs receptor chains
        Logic is based on output chains.
        Work out segments from the output chain letters. If ('_','_') in that segment, it's an inpaint chain
        """
        outchains=[i[0] for i in self.hal]
        receptor=[]
        inpaint=[]
        receptor_hal=[]
        inpaint_hal=[]
        for ch in np.unique(outchains):
            ch_mask=[i==ch for i in outchains]
            chain=[i for idx,i in enumerate(self.ref) if ch_mask[idx]]
            halchain=[i for idx,i in enumerate(self.hal) if ch_mask[idx]]
            if ('_','_') in chain:
                # inpaint chain
                inpaint.extend(chain)
                inpaint_hal.extend(halchain)
            else:
                receptor.extend(chain)
                receptor_hal.extend(halchain)
        self.receptor=receptor
        self.inpaint=inpaint
        self.receptor_hal=receptor_hal
        self.inpaint_hal=inpaint_hal
